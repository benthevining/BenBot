...
namespace {
    ...
    [[nodiscard]] constexpr std::array<Bitboard, 64uz> calculate_bishop_masks() noexcept
    {
        std::array<Bitboard, 64uz> result {};

        for (int i = 0; i < 64; ++i) {
            const auto sq = Square::from_index(i);

            const int file = std::to_underlying(sq.file);
            const int rank = std::to_underlying(sq.rank);
    ...
    [[nodiscard]] constexpr std::array<Bitboard, 64uz> calculate_rook_masks() noexcept
    {
        std::array<Bitboard, 64uz> result {};
    ...
    [[nodiscard]] constexpr Bitboard calculate_bishop_moves(const Square& sq, const Bitboard blockers)
    {
        Bitboard result;
        ...
        for (int y = rank + 1, x = file + 1; y <= 7 && x <= 7; ++y, ++x) {
            const auto nsq = Square::from_index(x + y * 8);
            const auto bb  = Bitboard::from_square(nsq);
            result |= bb;
    ...
    [[nodiscard]] constexpr Bitboard calculate_rook_moves(const Square& sq, const Bitboard blockers)
    {
        Bitboard result;
    ...
    [[nodiscard]] constexpr Bitboard permute(const Bitboard set, const Bitboard subset)
    {
    ...
    {
        std::array<std::uint64_t, 88772uz> result {};
        ...
        for (int i = 0; i < 64; ++i) {
            ...
            do {
                ...
                *index = calculate_bishop_moves(sq, perm).to_int();
            ...
            do {
                ...
                *index = calculate_rook_moves(sq, perm).to_int();
    ...
    [[nodiscard]] Bitboard bishop_moves(const Square& bishopPos, const Bitboard occupied) noexcept
    {
...
Bitboard bishop(
    const Square& bishopPos, const Bitboard occupiedSquares, const Bitboard friendlyPieces) noexcept
{
    return bishop_moves(bishopPos, occupiedSquares) & friendlyPieces.inverse();
}

Bitboard rook(
    const Square& rookPos, const Bitboard occupiedSquares, const Bitboard friendlyPieces) noexcept
{
    return rook_moves(rookPos, occupiedSquares) & friendlyPieces.inverse();
}

Bitboard queen(
    const Square& queenPos, const Bitboard occupiedSquares, const Bitboard friendlyPieces) noexcept
{
