...
for position in CORRECT_DATA:
    startingFEN = position['position']

    print(f'Running tests for position {startingFEN}')

    for depthObj in position['depths']:
        depth = depthObj['depth']
        correctResultObj = depthObj['results']

        results_file = TMP_DIR_PATH / f'position_{num_positions}' / f'{depth}_results.json'
