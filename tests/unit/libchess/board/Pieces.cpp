...
TEST_CASE("Pieces - is_file_half_open()", TAGS)
{
    ...
    for (const auto file : magic_enum::enum_values<File>())
        REQUIRE_FALSE(pieces.is_file_half_open(file));
...
TEST_CASE("Pieces - has_bishop_pair()", TAGS)
{
    ...
    REQUIRE_FALSE(pieces.has_bishop_pair());
...
TEST_CASE("Pieces - get_piece_on()", TAGS)
{
    ...
    STATIC_REQUIRE_FALSE(pieces.get_piece_on(Square { File::A, Rank::Three }).has_value());
    STATIC_REQUIRE_FALSE(pieces.get_piece_on(Square { File::C, Rank::Four }).has_value());
    STATIC_REQUIRE_FALSE(pieces.get_piece_on(Square { File::E, Rank::Eight }).has_value());
    STATIC_REQUIRE_FALSE(pieces.get_piece_on(Square { File::H, Rank::Five }).has_value());
