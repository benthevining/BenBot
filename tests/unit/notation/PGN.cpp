...
static constexpr std::string_view examplePGN {
...
TEST_CASE("PGN - parsing metadata", TAGS)
{
    const auto game = from_pgn(examplePGN);
    ...
    REQUIRE(to_pgn(game) == std::string { examplePGN });
}
