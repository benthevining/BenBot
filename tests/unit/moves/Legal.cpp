...
TEST_CASE("Maximum known moves in a position", TAGS)
{
    ...
    const auto moves = generate(position) | std::ranges::to<std::vector>();
...
TEST_CASE("Captures only", TAGS)
{
    ...
    SECTION("Kiwipete")
    {
        ...
        const auto moves = generate<true>(position) | std::ranges::to<std::vector>();

        REQUIRE(moves.size() == 8uz);
    }
